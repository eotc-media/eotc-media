...
const form = useForm({
    language_id: props.hymn.languages.map(language => language.id),
    category_id: props.hymn.categories.map(category => category.id),
    sub_category_id: props.hymn.sub_categories.map(sub_category => sub_category.id),
    singer_id: props.hymn.singers.map(singer => singer.id),
    singer: props.hymn.singer,
    description: props.hymn.description,
})
...
const submit = (id) => {
    form.put(route('hymn.admin.hymns.update', id), {
        onFinish: () => {
            form.reset()
        },
    })
...
<template>
    ...
    <Head title="Edit Hymn" />

    <div class="container-fluid py-3">
        <form @submit.prevent="submit(hymn.id)">
            <div class="card">
                <div class="card-header">
                    <h4 class="font-weight-normal">Edit New Hymn</h4>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-lg-6">
                        ...
                        <div class="col-lg-6">
                            <div class="container-fluid form-group">
                                <div for="video_id"><b>Video ID:</b> {{ hymn.video_id }}
                                </div>
                            </div>
                            <div class="container-fluid form-group">
                            ...
                            <label for="singer_id">Hymn singer/s (Standard)</label>
                            <div class="container-fluid form-group mb-3">
                                <v-select multiple :options="singers" label="name" :reduce="singer => singer.id"
                                    v-model="form.singer_id">
                                </v-select>
                            </div>
                            <InputError :message="form.errors.singer_id"></InputError>
                        </div>
                    </div>
                    <div class="container-fluid form-group">
                        ...
                        <div class="input-group mb-3">
                            <textarea id="description" class="form-control" v-model="form.description"
                                name="description" placeholder="Description (optional)" autocomplete="description"
            ...
            <div class="text-right pt-3 mb-3">
                <Link :href="route('hymn.admin.hymns.index')" class="btn btn-outline-primary mr-2">Back</Link>
